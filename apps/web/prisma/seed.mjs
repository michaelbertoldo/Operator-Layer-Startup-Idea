...
async function main() {
  ...
  await prisma.spendAnomalyEvent.deleteMany();
  ...
  await prisma.agentApiKey.createMany({
    data: [
      createAgentApiKey("key_research", "agt_research", "Research demo key"),
      createAgentApiKey("key_sales", "agt_sales", "Sales demo key"),
      createAgentApiKey("key_ops", "agt_ops", "Ops demo key")
    ]
  });
...
function createSeedAuditLogs(events) {
  let prevHash = null;

  return events.map((event) => {
    const hash = hashAuditEvent({ ...event, prevHash });
    const row = {
      ...event,
      prevHash,
      hash
